...
in database
add the newhouse and customer database
"""
...
def my_addnewhouse_page():
    ...
    entered_avg_areapopulation = flask.request.form.get('apopulation')
    ...
    # if user not exists then add new record to database and return account created successfully
    my_query = f"INSERT INTO NEWHOUSE_TABLE( CUSTOMERNAME,CUSTOMERCONTACT, AREA,ADDRESS,AVG_AREAINCOME,AVG_AREAHOUSEAGE,AVG_AREANUMBEROFROOMS,AVG_AREANUMBEROFBEDROOMS,AREAPOPULATION,PRICE,REMARKS) VALUES('{entered_customername}', '{entered_customercontact}', '{entered_area}', '{entered_address}', '{entered_avg_areaincome}', '{entered_avg_areahouseage}', '{entered_avg_areanumberofrooms}', '{entered_avg_areanumberofbedrooms}', '{entered_avg_areapopulation}', '{entered_price}', '{entered_remarks}')"
    my_db_cursor.execute(my_query)
    my_db_connection.commit()
    my_db_connection.close()
